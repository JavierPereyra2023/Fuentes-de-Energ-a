...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      note.className = 'note';
    note.textContent = 'Arrastrá para rotar · rueda para acercar · clic derecho para desplazar';
    ...
    async _boot() {
      ...
      // Modo UMD: usar globals inyectados por <script> previos
      if (typeof THREE === 'undefined') throw new Error('three-d-stage: THREE global no encontrado — cargá three.min.js antes que este archivo.');
      if (typeof THREE.OrbitControls === 'undefined') throw new Error('three-d-stage: THREE.OrbitControls no encontrado — cargá OrbitControls.js antes que este archivo.');
      const controlsMod = { OrbitControls: THREE.OrbitControls };
      this._THREE = THREE;
      ...
      const mobileDpr = window.matchMedia('(max-width: 720px)').matches ? 1.35 : 2;
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, mobileDpr));
      ...
      scene.add(new THREE.HemisphereLight(0xdce8ff, 0x1b2436, 0.85));
    ...
    async _exportObj() {
      if (!this._object) return;
      const mod = (typeof THREE !== 'undefined' && THREE.OBJExporter) ? { OBJExporter: THREE.OBJExporter } : null; if (!mod) { console.warn('OBJExporter no disponible en build UMD; descarga deshabilitada'); return; }
    ...
    async _exportGlb() {
      if (!this._object) return;
      const mod = (typeof THREE !== 'undefined' && THREE.GLTFExporter) ? { GLTFExporter: THREE.GLTFExporter } : null; if (!mod) { console.warn('GLTFExporter no disponible en build UMD; descarga deshabilitada'); return; }
      this._nameParts();
      const base = this._basename;
