...
- **Modelo 3D de la central nuclear** (`modelo3D-central_nuclear/central-nuclear-3d.html`, ~2.600 mallas, ~60 fps): a diferencia de los otros tres modelos, este fue extendido y ya no es copia literal del origen. Detalles a respetar si se lo edita:
  - **Ciclo del día**: la escena no tiene un toggle día/noche sino un reloj continuo (`hora` 0–24, slider + chips Amanecer/Mediodía/Atardecer/Noche + botón "Ciclo automático", `CICLO_SEG = 96` s por día). La paleta vive en el array `CIELO` (10 paradas horarias con cielo/niebla/hemi/key/fill) y se interpola en `aplicarHora(h)`. El sol sale por +x a las 6 y se pone a las 18 (`direccionAstro`); la luna va 12 h desfasada. La luz principal (`key`) sigue al astro que está sobre el horizonte. `oscuridad` (0 día → 1 noche) es la variable global que gobierna ventanas, farolas, faros y estrellas.
  - **Sol y luna**: `astro()` arma disco + halo con una `CanvasTexture` de degradado radial (`texResplandor`, aditiva) — nada de archivos externos. El sol se agranda y enrojece cerca del horizonte; la luna lleva manchas hijas del disco. Ambos hacen `lookAt(camera)` en el tick.
  - **Cielo estrellado**: tres capas de `Points` (finas, brillantes y una banda inclinada que insinúa la Vía Láctea), con `fog: false` y titileo global.
  - **Ciudad y pueblo**: `PLANO` mapea manzana (`i,j`) → `plaza | iglesia | escuela | cancha | torres | medio | galpones`, y todo lo demás es `casas`. `casa()` genera casas de techo a dos aguas (dos faldones inclinados 30°), jardín, cerco, chimenea, ventanas emisivas y farol de entrada. Al final se marca **toda** la ciudad con `userData.sinRealce = true`: si no, seleccionar "red eléctrica" tiñe de verde el pueblo entero.
