...
├── presentacion.html           # Presentación del proyecto en 9 diapositivas (cian #00E5FF) — enlazada desde la nav de inicio.html
...
- **`presentacion.html` en el celular (agosto 2026)**: ~~la presentación usaba `scroll-snap-type: y mandatory/proximity` con `.slide { min-height: 100dvh/100svh }` y animación de entrada `.reveal` (opacity 0 hasta que el `IntersectionObserver` marcaba la diapositiva como `.active`). En celular varias diapositivas quedan más altas que la pantalla: el snap volvía inalcanzable el contenido de abajo y, si el observer —que usaba `threshold: [.45,.6,.75]`, es decir porcentaje de área visible— nunca activaba la diapositiva, todo el texto se quedaba en `opacity: 0` y la presentación se veía vacía.~~ Corregido con tres cambios:
  1. `@media (max-width: 900px)`: `scroll-snap-type: none`, `.slide { min-height: auto }` (salvo `.cover`) y `.reveal` siempre visible. En celular la presentación se lee como página normal.
  2. El observer pasó a línea central (`rootMargin: '-45% 0px -45% 0px'`, `threshold: 0`): la diapositiva activa es la que cruza el medio de la pantalla, sin depender de su altura. Si no hay `IntersectionObserver`, se agrega `reveal-all` al `<html>`.
  3. El canvas decorativo (`#probabilityField`) quedó opcional (`getContext('2d')` puede devolver `null` en celulares con poca memoria y antes cortaba el resto del script).

- **Texto que tapaba el cohete 3D del Falcon Heavy**: ~~`conceptos/modelo3D_rocket/falcon-heavy-3d.html` posiciona el título y la tabla de especificaciones sobre el canvas con `position: absolute`, pensado para la maqueta de escritorio (texto a la izquierda, cohete a la derecha). En pantallas angostas el canvas centra el cohete y el overlay le quedaba encima.~~ Corregido en `rocket-viewer-theme.css` (hoja compartida que los tres visores cargan al final del `<head>`), acotado con `[data-sc-name="falcon-heavy-3d"]`: ≤900 px achica el título, oculta la descripción ("pasá el cursor…", inútil en táctil) y compacta las specs a dos columnas; ≤560 px oculta la tabla entera (los mismos datos ya están en la ficha de `energia-espacial.html`, debajo del iframe). Como el overlay lo genera React con estilos inline, **todas las reglas van con `!important`** y se seleccionan por `data-dc-tpl` (7 título, 8 volanta, 9 nombre, 10 descripción, 11 panel de specs, 12 rótulo, 14 fila, 28 hint). Al tocar esa hoja hay que subir el cache-buster `?v=mobileN` en los tres visores (`falcon-heavy-3d.html`, `falcon-9/falcon9-3d.html`, `starship/starship.html`).
